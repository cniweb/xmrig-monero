...
if docker run --rm --entrypoint="" "$IMAGE" grep -r "access-token" /home/xmrig/config.json | grep -q '"access-token": null'; then
    echo "   PASS: No hardcoded access tokens found"
else
...
PERMS=$(docker run --rm --entrypoint="" "$IMAGE" ls -la /home/xmrig/start_zergpool.sh /home/xmrig/xmrig 2>/dev/null)
if [ $? -eq 0 ]; then
